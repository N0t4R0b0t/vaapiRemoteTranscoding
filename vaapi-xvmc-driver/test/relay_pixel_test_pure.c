/*
 * Same as relay_pixel_test.c, but drives the relay path with a
 * synthetic, controlled "pure" source (a locally x264-encoded SMPTE
 * color-bars clip, see relay_pixel_test_slices_pure.h) instead of real
 * broadcast capture content -- to test whether the magenta/pink tint
 * seen on real hardware is specific to real-world capture content
 * (unusual matrices, chroma subsampling artifacts, etc.) or reproduces
 * identically on known, simple, non-broadcast content too.
 *
...
#include "relay_pixel_test_slices_pure.h"
...
/* Real VAPictureParameterBufferH264 field values for this test's own
 * locally-generated encode (see relay_pixel_test_slices_pure.h),
 * confirmed against its actual SPS/PPS via ffmpeg -bsf:v trace_headers,
 * not guessed. 352x288 = 22x18 macroblocks. Only num_ref_frames differs
 * from relay_pixel_test.c's real-capture values (this encode used
 * ref=1); every other field came out identical. */
...
int main(int argc, char **argv)
{
    ...
    /* Synthetic, controlled content -- a locally x264-encoded SMPTE
     * color-bars clip (see relay_pixel_test_slices_pure.h), not real
     * broadcast capture. Same I,P,P-repeated-mini-GOP replay strategy
     * as relay_pixel_test.c (see that file for why: an IDR always
     * resets reference-picture state, so repeating I,P,P,I,P,P,... is
     * legitimate H.264, and the steady realistic pacing is what lets
     * relay-server's own ffmpeg stream analysis produce output at all). */
    const unsigned char *long_slices[3] = { pure_slice0_bytes, pure_slice1_bytes, pure_slice2_bytes };
    size_t long_slice_lens[3] = { sizeof(pure_slice0_bytes), sizeof(pure_slice1_bytes),
                                   sizeof(pure_slice2_bytes) };
